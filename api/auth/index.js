...
module.exports = (req, res) => {
  try {
    ...
    const cleanPartnerKey = partnerKey.trim().replace(/\r?\n|\r/g, '');  // Limpa quebras de linha invisíveis
    const baseString = `${partner_id}${path}${timestamp}`;
    const sign = crypto.createHmac('sha256', cleanPartnerKey).update(baseString).digest('hex');
